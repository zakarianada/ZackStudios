...
function FeaturedVideo() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting && !mobileQuery.matches) video.play().catch(() => setPaused(true));
      else video.pause();
    }, { threshold: .3 });
  ...
  return (
    <div ref={containerRef} className="group relative size-full bg-black fullscreen:h-screen fullscreen:w-screen">
      ...
      >
        <source src="/media/zack-showreel-mobile.mp4" type="video/mp4" media="(max-width: 767px)" />
      ...
      <div className="absolute inset-x-0 bottom-0 hidden items-end justify-between gap-3 p-6 md:flex">
        <div className="pointer-events-none">
          <p className="text-[9px] font-semibold uppercase tracking-[.2em] text-[#ff3439]">Featured showreel</p>
          <p className="mt-1 text-xs text-white/60">{paused ? "Press play to watch" : muted ? "Playing silently" : "Sound on"}</p>
        </div>
...
export function MotionShowcase() {
  ...
  const dialogRef = useDialog(Boolean(active), closeProject);
  ...
  return (
    <section id="motion" className="relative overflow-hidden border-b border-white/8 bg-[#090909]">
      ...
      <div className="relative z-10 mx-auto mt-12 max-w-[1500px] px-5 pb-20 md:-mt-32 md:px-9 md:pb-24">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {motionProjects.slice(1).map((project, index) => (
            <button key={project.id} onClick={() => setActive(project)} className="group relative overflow-hidden rounded-xl border border-white/10 bg-[#0d0d0e] p-0 text-left transition hover:border-[#ff3439]/55" aria-label={`Play ${project.title}`}>
      ...
      <AnimatePresence>
        {active && (
          <motion.div className={`fixed inset-0 z-[80] grid place-items-center bg-black/92 backdrop-blur-xl ${widePlayer ? "p-0" : "p-0 sm:p-4"}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={closeProject}>
            <motion.div ref={dialogRef} role="dialog" aria-modal="true" aria-label={`${active.title} video player`} className={`flex w-full max-w-6xl flex-col overflow-hidden bg-[#090909] ${widePlayer ? "h-[100svh] rounded-none" : "h-[100svh] rounded-none border-0 sm:h-auto sm:max-h-[calc(100svh-2rem)] sm:rounded-2xl sm:border sm:border-white/12"}`} initial={{ scale: .94, y: 24 }} animate={{ scale: 1, y: 0 }} exit={{ scale: .96, y: 18 }} transition={{ type: "spring", damping: 24 }} onClick={(e) => e.stopPropagation()}>
              <div className="flex min-h-16 shrink-0 items-center justify-between border-b border-white/10 px-[max(.75rem,env(safe-area-inset-left))] py-2.5 pr-[max(.75rem,env(safe-area-inset-right))] sm:px-5 sm:py-4"><div className="min-w-0"><p className="truncate text-[9px] uppercase tracking-[.16em] text-[#ff3439] sm:text-[10px] sm:tracking-[.18em]">{active.type}</p><h3 className="truncate text-sm font-semibold sm:text-base">{active.title}</h3></div><div className="ml-2 flex shrink-0 gap-1.5 sm:gap-2"><a href={`https://drive.google.com/file/d/${active.driveId}/view`} target="_blank" rel="noopener noreferrer" className="grid size-11 place-items-center rounded-full border border-white/12 hover:bg-white/5" aria-label="Open video directly"><ExternalLink className="size-4" /></a><button onClick={openProjectFullscreen} className="inline-flex h-11 items-center gap-2 rounded-full border border-white/12 px-3 text-[9px] font-semibold uppercase tracking-[.08em] hover:bg-white/5" aria-label="View video in wide screen"><Maximize2 className="size-4" /><span className="hidden min-[360px]:inline">Wide screen</span></button><button onClick={closeProject} className="grid size-11 place-items-center rounded-full border border-white/12 hover:bg-white/5" aria-label="Close video"><X className="size-4" /></button></div></div>
              <div ref={playerRef} className={`${widePlayer ? "min-h-0 flex-1" : "min-h-0 flex-1 sm:flex-none sm:aspect-video"} w-full bg-black fullscreen:h-screen fullscreen:w-screen fullscreen:aspect-auto`}><VideoFrame key={active.id} project={active} /></div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
