    ...
    <div className="group relative size-full bg-black">
      ...
      <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-3 p-4 md:p-6">
        <div className="pointer-events-none">
          ...
          <p className="mt-1 text-xs text-white/60">Autoplaying silently</p>
        </div>
        <div className="flex gap-2">
          <button onClick={togglePlayback} className="inline-flex h-11 items-center gap-2 rounded-full border border-white/18 bg-black/70 px-4 text-[9px] font-semibold uppercase tracking-[.12em] text-white backdrop-blur-md transition hover:border-[#ff3439]/70" aria-label={paused ? "Play showreel" : "Pause showreel"}>
            {paused ? <Play className="size-3.5 fill-current" /> : <Pause className="size-3.5 fill-current" />}
            {paused ? "Play" : "Pause"}
          </button>
          <button onClick={toggleSound} className="inline-flex h-11 items-center gap-2 rounded-full border border-white/18 bg-black/70 px-4 text-[9px] font-semibold uppercase tracking-[.12em] text-white backdrop-blur-md transition hover:border-[#ff3439]/70" aria-label={muted ? "Unmute showreel" : "Mute showreel"}>
        ...
        {active && (
          <motion.div className="fixed inset-0 z-[80] grid place-items-center bg-black/92 p-4 backdrop-blur-xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setActive(null)}>
            <motion.div className="w-full max-w-6xl overflow-hidden rounded-2xl border border-white/12 bg-[#090909]" initial={{ scale: .94, y: 24 }} animate={{ scale: 1, y: 0 }} exit={{ scale: .96, y: 18 }} transition={{ type: "spring", damping: 24 }} onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between border-b border-white/10 px-5 py-4"><div><p className="text-[10px] uppercase tracking-[.18em] text-[#ff3439]">{active.type}</p><h3 className="font-semibold">{active.title}</h3></div><button onClick={() => setActive(null)} className="grid size-10 place-items-center rounded-full border border-white/12 hover:bg-white/5" aria-label="Close video"><X className="size-4" /></button></div>
              <div className="aspect-video"><VideoFrame project={active} /></div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
